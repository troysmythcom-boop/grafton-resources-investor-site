...
test('production server serves pages, approved sources, grounded answers and honest disconnected forms',async()=>{
 ...
 const answer=await (await post('/api/chat',{question:'Who is on the management team?',lang:'en'})).json();assert.match(answer.answer,/Campbell/i);assert.ok(answer.sources.length);
