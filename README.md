...
A six-page, responsive investor website with an original mineral-blue visual direction, company logo, generated landscape and mineral artwork, scroll effects, interactive 3D terrain, market charts and a capital structure visualisation.
...
This is a local redesign preview, not a modification of the live company website.
...
For static hosting, serve the `index.html` fallback for `/projects`, `/investors`, `/company`, `/news` and `/contact`, and publish `styles.css`, `app.js`, `terrain.js` and the `assets` directory. Do not expose the local preview server publicly.
...
The presentation and announcement buttons link to the documents provided by the original website. Company logo: Grafton Resources. Landscape and mineral artwork: generated for this design. Fonts: Barlow Condensed and Hanken Grotesk, served by Google Fonts.
